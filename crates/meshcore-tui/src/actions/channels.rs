...
pub fn reload(state: Arc<AppState>, action_tx: UnboundedSender<Action>) {
    tokio::spawn(async move {
        let result = meshcore_service::channels::get_all_channels(&state);
        match result {
            Ok(list) => {
...
pub fn spawn_mark_read(
    ...
    tokio::spawn(async move {
        let result = meshcore_service::channels::mark_as_read(&state, channel_idx);
        if let Err(e) = result {
            let _ = action_tx.send(Action::Async(AsyncResult::Generic(Err(e))));
            return;
        }
...
pub fn spawn_sync_to_device(
    state: Arc<AppState>,
    channel: meshcore_storage::channels::StoredChannel,
    action_tx: UnboundedSender<Action>,
) {
    tokio::spawn(async move {
        ...
        let result = meshcore_service::channels::sync_channel_to_device(
            &state,
            channel.idx,
            &channel.name,
            &psk_array,
        ...
        let msg = match result {
            Ok(()) => Ok(format!("Canal « {} » synchronisé sur le device", channel.name)),
...
pub fn spawn_delete(
    ...
    tokio::spawn(async move {
        let result = meshcore_service::channels::delete_channel(&state, channel_idx).await;
        let msg = match result {
            Ok(()) => Ok("Canal supprimé".to_string()),
