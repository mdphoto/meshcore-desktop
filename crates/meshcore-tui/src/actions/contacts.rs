...
pub fn spawn_sync(state: Arc<AppState>, action_tx: UnboundedSender<Action>) {
    tokio::spawn(async move {
        let result = meshcore_service::contacts::sync_contacts(&state).await;
...
pub fn reload(state: Arc<AppState>, action_tx: UnboundedSender<Action>) {
    tokio::spawn(async move {
        let result = meshcore_service::contacts::get_all_contacts(&state);
        match result {
            Ok(list) => {
...
pub fn sort(list: &mut [meshcore_storage::models::StoredContact], mode: crate::state::app_state::ContactSortMode) {
...
pub fn build_rows(
    contacts: &[meshcore_storage::models::StoredContact],
...
pub fn spawn_toggle_fav(state: Arc<AppState>, pubkey: String, favorite: bool, action_tx: UnboundedSender<Action>) {
    tokio::spawn(async move {
        match meshcore_service::contacts::toggle_favorite(&state, &pubkey, favorite) {
...
pub fn spawn_delete(state: Arc<AppState>, pubkey: String, action_tx: UnboundedSender<Action>) {
    tokio::spawn(async move {
        let result = meshcore_service::contacts::delete_contact(&state, &pubkey).await;
        let msg = match result {
            Ok(()) => Ok("Contact supprimé".to_string()),
            Err(e) => Err(e),
        };
