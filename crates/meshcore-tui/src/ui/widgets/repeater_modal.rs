...
use crate::theme;
use meshcore_service::repeater::RepeaterStatus;
use ratatui::{
    Frame,
...
fn render_status(frame: &mut Frame, area: Rect, ui: &RepeaterUiState) {
    ...
    let lines: Vec<Line> = match (&ui.status, &ui.status_error) {
        ...
        (None, Some(err)) => error_lines("Erreur lors de la récupération du statut", err),
        (None, None) => vec![
            Line::from(""),
            Line::from(Span::styled(
...
fn status_lines(s: &RepeaterStatus) -> Vec<Line<'static>> {
    vec![
        line_kv("Batterie", &format!("{} mV", s.battery_mv)),
        line_kv("TX queue", &format!("{}", s.tx_queue_len)),
        line_kv("Noise floor", &format!("{} dBm", s.noise_floor)),
        line_kv("Last RSSI", &format!("{} dBm · SNR {:.1}", s.last_rssi, s.snr)),
        line_kv(
            "Traffic",
            &format!(
        ...
        line_kv("Uptime", &format!("{} s", s.uptime)),
        line_kv("Duplicates", &format!("{}", s.dup_count)),
    ]
}
...
fn render_neighbours(frame: &mut Frame, area: Rect, ui: &RepeaterUiState) {
    ...
    if ui.neighbours.is_empty() {
        if let Some(err) = &ui.neighbours_error {
            frame.render_widget(
                Paragraph::new(error_lines("Erreur lors de la récupération des voisins", err))
                    .block(block)
...
fn render_acl(frame: &mut Frame, area: Rect, ui: &RepeaterUiState) {
    ...
    if ui.acl.is_empty() {
        if let Some(err) = &ui.acl_error {
            frame.render_widget(
                Paragraph::new(error_lines("Erreur lors de la récupération de l'ACL", err))
                    .block(block)
                    .wrap(Wrap { trim: false }),
                area,
            );
