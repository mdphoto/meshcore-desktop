...
pub async fn scan_ble_devices(duration_secs: Option<u64>) -> Result<Vec<BleDevice>, String> {
    ...
    {
        use uuid::Uuid;
        let meshcorex_service_uuid = Uuid::from_u128(0x6e400001_b5a3_f393_e0a9_e50e24dcca9e);
        adapter
            .start_scan(ScanFilter {
                services: vec![meshcorex_service_uuid],
            })
            .await
...
pub async fn connect_serial(
    ...
    meshcorex_service::connection::connect(&state, ConnectionTarget::Serial { port, baud_rate })
        .await
}
...
pub async fn connect_tcp(
    ...
    meshcorex_service::connection::connect(&state, ConnectionTarget::Tcp { host, port }).await
}

#[tauri::command]
pub async fn connect_ble(state: State<'_, AppState>, name_or_addr: String) -> Result<(), String> {
    meshcorex_service::connection::connect(&state, ConnectionTarget::Ble { name_or_addr }).await
}
...
pub async fn is_connected(state: State<'_, AppState>) -> Result<bool, String> {
    Ok(meshcorex_service::connection::is_connected(&state).await)
}
