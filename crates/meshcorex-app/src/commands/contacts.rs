...
pub async fn sync_contacts(state: State<'_, AppState>) -> Result<usize, String> {
    meshcorex_service::contacts::sync_contacts(&state).await
}

#[tauri::command]
pub fn get_all_contacts(state: State<'_, AppState>) -> Result<Vec<StoredContact>, String> {
    meshcorex_service::contacts::get_all_contacts(&state)
}
...
pub fn toggle_favorite(
    ...
    meshcorex_service::contacts::toggle_favorite(&state, &public_key, favorite)
}

#[tauri::command]
pub async fn delete_contact(state: State<'_, AppState>, public_key: String) -> Result<(), String> {
    meshcorex_service::contacts::delete_contact(&state, &public_key).await
}
