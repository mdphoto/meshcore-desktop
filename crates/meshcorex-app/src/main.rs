//! Point d'entrée de l'application MeshCoreX (GUI legacy)
...
fn main() {
    ...
    tracing::info!("Démarrage de MeshCoreX (GUI legacy)");
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                let db = meshcorex_storage::Database::open(&db_path).ok();
                while let Ok(ref event) = rx.recv().await {
                    // Stocker les messages reçus en DB
                    if let Some(ref db) = db {
                        match event {
                            meshcorex_service::AppEvent::DirectMessageReceived {
                                sender_pubkey,
                                sender_name,
                                text,
                                snr,
                            ...
                            meshcorex_service::AppEvent::ChannelMessageReceived {
                                channel_idx,
                                sender_name,
                                text,
                            } => {
