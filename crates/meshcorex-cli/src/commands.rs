...
pub async fn execute_subcommand(
    ...
    match cmd {
        SubCommand::Contacts { action } => match action.unwrap_or(ContactsAction::List) {
            ContactsAction::Sync => {
                let count = meshcorex_service::contacts::sync_contacts(state).await?;
                if json {
            ...
            ContactsAction::List => {
                let contacts = meshcorex_service::contacts::get_all_contacts(state)?;
        ...
        SubCommand::Send { dest, message } => {
            ...
            let id =
                meshcorex_service::messaging::send_direct_message(state, &pubkey, &text).await?;
            if json {
                println!("{{\"sent\":\"{}\"}}", id);
            } else {
        ...
        SubCommand::Channel { idx, message } => {
            let text = message.join(" ");
            let id = meshcorex_service::messaging::send_channel_message(state, idx, &text).await?;
            if json {
                println!("{{\"sent\":\"{}\"}}", id);
            } else {
        ...
        SubCommand::Channels => {
            let channels = meshcorex_service::channels::get_all_channels(state)?;
            display::channels(&channels, json);
        }
        SubCommand::Device => {
            let info = meshcorex_service::device::get_device_info(state).await?;
        ...
        SubCommand::Battery { chemistry } => {
            ...
            let (mv, pct) = meshcorex_service::device::get_battery(state, chem).await?;
...
pub async fn execute_repl(state: &AppState, tokens: &[String], json: bool) -> Result<bool, String> {
    ...
    match tokens[0].as_str() {
        ...
        "connect" => {
            ...
            meshcorex_service::connection::connect(state, target).await?;
            println!("Connecté");
        }

        "disconnect" => {
            meshcorex_service::connection::disconnect(state).await?;
            println!("Déconnecté");
        }

        "status" => {
            let connected = meshcorex_service::connection::is_connected(state).await;
            if json {
        ...
        "reboot" => {
            meshcorex_service::device::reboot(state).await?;
        ...
        "repeater" => {
            ...
            match tokens[1].as_str() {
                "login" => {
                    ...
                    let r =
                        meshcorex_service::repeater::login(state, &tokens[2], &tokens[3]).await?;
                    println!("{}", r);
                }
                "logout" => {
                    meshcorex_service::repeater::logout(state, &tokens[2]).await?;
                ...
                "status" => {
                    ...
                    for cmd in &["ver", "get name", "get radio", "get tx"] {
                        match meshcorex_service::repeater::send_cli(state, &tokens[2], cmd).await {
                ...
                "cli" => {
                    ...
                    let resp =
                        meshcorex_service::repeater::send_cli(state, &tokens[2], &cmd).await?;
                    println!("{}", resp);
                }
                "neighbours" => {
                    let n =
                        meshcorex_service::repeater::neighbours(state, &tokens[2], 50, 0).await?;
        ...
        "history" => {
            ...
            let msgs = meshcorex_service::messaging::get_direct_messages(state, &pubkey, limit, 0)?;
