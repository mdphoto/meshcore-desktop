...
#[command(
    name = "meshcorex",
    version,
    about = "MeshCoreX — TUI + CLI unifié pour dispositifs LoRa MeshCore"
)]
struct Cli {
    ...
    /// Répertoire de données (défaut: ~/.local/share/meshcorex/)
    #[arg(long)]
...
async fn main() {
    ...
    if let Some(ref port) = cli.port {
        let target = meshcorex_transport::manager::ConnectionTarget::Serial {
            port: port.clone(),
            baud_rate: cli.baud,
        };
        if let Err(e) = meshcorex_service::connection::connect(&state, target).await {
        ...
        if parts.len() == 2 {
            ...
            let target = meshcorex_transport::manager::ConnectionTarget::Tcp { host, port };
            if let Err(e) = meshcorex_service::connection::connect(&state, target).await {
        ...
        let target = meshcorex_transport::manager::ConnectionTarget::Ble {
    ...
    match cli.command {
        ...
        None => {
            if cli.repl {
                ...
                if let Err(e) = meshcorex_tui::run_tui(service, auto_reconnect).await {
