...
async fn build_prompt(state: &AppState) -> String {
    ...
    if connected {
        ...
        let name = conn
            .target()
            .map(|t| match t {
                meshcorex_transport::manager::ConnectionTarget::Serial { port, .. } => port.clone(),
                meshcorex_transport::manager::ConnectionTarget::Tcp { host, port } => {
                    format!("{}:{}", host, port)
                }
                meshcorex_transport::manager::ConnectionTarget::Ble { name_or_addr } => {
                    name_or_addr.clone()
                }
            })
