...
pub fn load_channel_senders(
    ...
    tokio::spawn(async move {
        ...
        if let Ok(stored) =
            meshcorex_service::messaging::get_channel_sender_names(&state, channel_idx, 100)
        {
            for n in stored {
        ...
        if let Ok(texts) =
            meshcorex_service::messaging::get_channel_incoming_texts(&state, channel_idx, 500)
        {
            for t in texts {
...
pub fn reload_dm_pubkeys(state: Arc<AppState>, action_tx: UnboundedSender<Action>) {
    tokio::spawn(async move {
        match meshcorex_service::messaging::get_dm_pubkeys(&state) {
            Ok(list) => {
    ...
    tokio::spawn(async move {
        ...
        let result = match &conversation {
            ConversationId::Dm(pk) => {
                meshcorex_service::messaging::get_direct_messages(&state, pk, PAGE_SIZE, offset)
            }
            ConversationId::Channel(idx) => {
                meshcorex_service::messaging::get_channel_messages(&state, *idx, PAGE_SIZE, offset)
            }
        };
        match result {
            Ok(mut list) => {
...
pub fn spawn_send_dm(
    ...
    tokio::spawn(async move {
        ...
        let msg = match result {
            ...
            Err(e) => Err(format!("{} : {}", crate::util::i18n::t("toast.send_failed"), e)),
        };
...
pub fn spawn_send_channel(
    ...
    tokio::spawn(async move {
        let result =
            meshcorex_service::messaging::send_channel_message(&state, channel_idx, &text).await;
        if let Err(e) = result {
            let _ = action_tx.send(Action::Async(AsyncResult::Generic(Err(format!(
                "{} : {}",
                crate::util::i18n::t("toast.channel_send_failed"),
                e
            )))));
        }
        load_messages(
