...
pub fn spawn_login(
    ...
    tokio::spawn(async move {
        let result = meshcorex_service::repeater::login(&state, &pubkey, &password).await;
...
pub fn spawn_logout(
    ...
    tokio::spawn(async move {
        let result = meshcorex_service::repeater::logout(&state, &pubkey).await;
        let msg = match result {
            Ok(()) => Ok(crate::util::i18n::t("toast.repeater_disconnected")),
            Err(e) => Err(e),
        };
...
pub fn spawn_status(
    ...
    tokio::spawn(async move {
        let result = meshcorex_service::repeater::status(&state, &pubkey).await;
...
pub fn spawn_neighbours(
    ...
    tokio::spawn(async move {
        let result = meshcorex_service::repeater::neighbours(&state, &pubkey, 32, 0).await;
...
pub fn spawn_acl(state: Arc<AppState>, pubkey: String, action_tx: UnboundedSender<Action>) {
    tokio::spawn(async move {
        let result = meshcorex_service::repeater::acl(&state, &pubkey).await;
...
pub fn spawn_cli(
    ...
    tokio::spawn(async move {
        let result = meshcorex_service::repeater::send_cli(&state, &pubkey, &command).await;
