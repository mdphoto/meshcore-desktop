...
use meshcorex_service::{AppEvent, AppState};
...
pub struct App {
    ...
    pub channels: Vec<meshcorex_storage::channels::StoredChannel>,
...
impl App {
    ...
    fn spawn_auto_reconnect(&self) {
        ...
        tokio::spawn(async move {
            ...
            match meshcorex_service::connection::connect(&svc, target).await {
                Ok(()) => {
    ...
    pub async fn run(
        ...
        // Charge la langue persistée depuis la DB settings (défaut FR)
        if let Ok(Some(code)) = self
            .service
            .db
            .with_conn(|c| meshcorex_storage::settings::get(c, "app.language"))
        {
            crate::util::i18n::set_lang(crate::util::i18n::Lang::from_code(&code));
        }
        ...
        conn_actions::reload_companions(self.service.clone(), self.action_tx.clone());
    ...
    fn dispatch(&mut self, action: Action) {
        match action {
            ...
            Action::ChatScrollUp => {
                let cur = self.chat_ui.active_scroll();
                self.chat_ui.set_active_scroll(cur.saturating_add(1));
            }
            Action::ChatScrollDown => {
                let cur = self.chat_ui.active_scroll();
                self.chat_ui.set_active_scroll(cur.saturating_sub(1));
            }
            ...
            Action::SettingsCycleLang => {
                use crate::util::i18n::{Lang, current_lang, set_lang};
                let next = match current_lang() {
                    Lang::Fr => Lang::En,
                    Lang::En => Lang::Fr,
                };
                set_lang(next);
                // Persiste dans la DB settings
                let code = next.code().to_string();
                let _ = self.service.db.with_conn(|c| {
                    meshcorex_storage::settings::set(c, "app.language", &code)
                });
                self.ui.toast(
                    format!("Langue : {}", next.label()),
                    ToastLevel::Success,
                );
            }
            ...
            Action::ConnDisconnectPrimary => {
                // Sur le sous-panneau Companions, `d` supprime le companion sélectionné
                // plutôt que de déconnecter le device actif.
                if matches!(
                    self.connection_ui.sub_pane,
                    ConnectionSubPane::Companions
                ) {
                    let sel = self
                        .connection_ui
                        .companions_list_state
                        .selected()
                        .unwrap_or(0);
                    if let Some(c) = self.connection_ui.companions.get(sel)
                        && let Some(id) = c.id
                    {
                        conn_actions::spawn_delete_companion(
                            self.service.clone(),
                            id,
                            self.action_tx.clone(),
                        );
            ...
            Action::ConnRequestPairBle => {
                // Résout le device BLE sélectionné (scan ou companion BLE)
                let target = match self.connection_ui.sub_pane {
                    ConnectionSubPane::BleScan => {
                    ...
                    ConnectionSubPane::Companions => {
                        ...
                        self.connection_ui
                            .companions
                            .get(sel)
                            .filter(|c| c.transport_type == "ble")
                            .map(|c| (c.address.clone(), c.name.clone()))
                    }
                    _ => None,
                };
                if let Some((addr, name)) = target {
                    self.ui.push_modal(ModalKind::PairBle { addr, name });
                } else {
                    self.ui
                        .toast("Sélectionnez un device BLE à pairer.", ToastLevel::Warn);
                }
            }
            Action::PairBleSubmit => {
                if let Some(ModalKind::PairBle { addr, name }) = self.ui.top_modal().cloned() {
                    self.ui.pop_modal();
                    self.ui.toast(
                        format!("Pairing {} en cours (répondre au popup OS si PIN requis)…", name),
                        ToastLevel::Info,
                    );
                    conn_actions::spawn_pair_ble(addr, self.action_tx.clone());
                }
            }
    ...
    fn chat_activate(&mut self, id: ConversationId) {
        ...
        // Scroll persistant par conversation : on ne reset QUE si c'est la première ouverture
        if !self.chat_ui.scroll_offsets.contains_key(&id) {
            self.chat_ui.scroll_offsets.insert(id.clone(), 0);
        }
    ...
    fn chat_send_current(&mut self) {
        ...
        self.chat_ui.input.reset();
        self.chat_ui.reset_active_scroll();
    }
    ...
    fn selected_channel(&self) -> Option<&meshcorex_storage::channels::StoredChannel> {
        self.channels
    ...
    fn submit_channel_new(&mut self) {
        ...
        let new_channel = meshcorex_storage::channels::StoredChannel {
            idx,
            name: name.clone(),
        ...
        if let Err(e) =
            meshcorex_service::channels::upsert_channel(&self.service, &new_channel)
        {
            self.ui
    ...
    fn submit_channel_edit(&mut self, sync_to_device: bool) {
        ...
        let updated = meshcorex_storage::channels::StoredChannel {
            idx,
            name: name.clone(),
        ...
        if let Err(e) = meshcorex_service::channels::upsert_channel(&self.service, &updated) {
            self.ui
                .toast(format!("Enregistrement local : {}", e), ToastLevel::Error);
            return;
        }
        ...
        if scope.is_empty() {
            let _ = self
                .service
                .db
                .with_conn(|c| meshcorex_storage::settings::delete(c, &scope_key));
            ...
            let _ = self
                .service
                .db
                .with_conn(|c| meshcorex_storage::settings::set(c, &scope_key, &scope_clone));
    ...
    fn connect_selected(&mut self) {
        match self.connection_ui.sub_pane {
            ...
            ConnectionSubPane::Companions => {
                let sel = self.connection_ui.companions_list_state.selected().unwrap_or(0);
                if let Some(c) = self.connection_ui.companions.get(sel).cloned()
                    && let Some(target) =
                        meshcorex_service::companions::companion_to_target(&c)
                {
                    self.ui.toast(
                        format!("Reconnexion à {}…", c.name),
                        ToastLevel::Info,
                    );
                    conn_actions::spawn_connect(
                        self.service.clone(),
        ...
        match self.ui.current_tab {
            ...
            Tab::Chat => match self.ui.focus {
                ...
                FocusTarget::ChatHistory => {
                    ...
                    self.chat_ui.set_active_scroll(new);
                }
                _ => {}
            },
            Tab::Channels => {
    ...
    fn handle_backend(&mut self, event: AppEvent) {
        match event {
            ...
            AppEvent::DirectMessageReceived {
                ...
                let msg = meshcorex_storage::models::StoredMessage {
                ...
                match self.service.db.with_conn(|c| {
                    meshcorex_storage::messages::insert_message(c, &msg)
                }) {
    ...
    fn handle_async(&mut self, result: AsyncResult) {
        match result {
            ...
            AsyncResult::ChannelsReloaded(list) => {
                ...
                for ch in &self.channels {
                    ...
                    if let Ok(Some(val)) = self
                        .service
                        .db
                        .with_conn(|c| meshcorex_storage::settings::get(c, &key))
                        && !val.is_empty()
                    {
            ...
            AsyncResult::CompanionsListed(list) => {
                self.connection_ui.companions = list;
                if self.connection_ui.companions.is_empty() {
                    self.connection_ui.companions_list_state.select(None);
                } else if self.connection_ui.companions_list_state.selected().is_none() {
                    self.connection_ui.companions_list_state.select(Some(0));
                }
            }
