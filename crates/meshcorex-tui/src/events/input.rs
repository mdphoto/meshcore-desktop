...
fn map_modal_key(key: KeyEvent, ui: &AppUiState) -> Action {
    match ui.top_modal() {
        ...
        Some(ModalKind::PairBle { .. }) => match key.code {
            KeyCode::Esc | KeyCode::Char('n') | KeyCode::Char('N') => Action::CloseModal,
            KeyCode::Enter | KeyCode::Char('y') | KeyCode::Char('Y') => Action::PairBleSubmit,
            _ => Action::NoOp,
        },
...
fn map_tab_key(key: KeyEvent, ui: &AppUiState) -> Action {
    ...
    match key.code {
        ...
        KeyCode::F(6) => return Action::GotoTab(Tab::Settings),
    ...
    match ui.current_tab {
        ...
        Tab::Settings => map_settings_key(key),
    }
}
...
fn digit_to_tab(code: KeyCode) -> Option<Tab> {
    match code {
        ...
        KeyCode::Char('6') => Some(Tab::Settings),
        _ => None,
    }
}
...
fn map_connection_key(key: KeyEvent, ui: &AppUiState) -> Action {
    ...
    match key.code {
        ...
        KeyCode::Char('P') => Action::ConnRequestPairBle, // pair BLE (majuscule pour éviter conflit)
